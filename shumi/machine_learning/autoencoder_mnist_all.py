...
print(len(test_data))
train_input, train_label = train_data.data, train_data.targets
train_input = train_input.float() / 255
train_flatten_input = torch.flatten(train_input, start_dim=1, end_dim=2)
...
hidden_size = 64
epochs = 200
save_path = 'result/compression_autoencoder_all_h{0}'.format(hidden_size)
os.makedirs(save_path, exist_ok=True)
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
mnist_sample = train_flatten_input[:64].to(device)

train_dataset = mydataset(len(train_flatten_input), train_flatten_input, train_label)
train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
#mnist_sample = copy.deepcopy(train_input_data[:64]).to(device)
batch_num = math.ceil(train_label.shape[0] / batch_size)
